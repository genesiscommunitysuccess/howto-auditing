...
To enable auditing on a table, specify the parameter `audit = details(id = X, sequence = Y)` on it within the tables dictionary [`howto-auditing-tables-dictionary.kts`](server/howto-auditing-app/src/main/genesis/cfg/howto-auditing-tables-dictionary.kts). Here you can see the audit table’s unique ID is set to 11_500 and records will have their generated id [sequence](https://learn.genesis.global/docs/previous/database/fields-tables-views/fields/tables-advanced/?_highlight=sequence#automatically-generated-sequences) contain TA. This will create a table with name TRADE_AUDIT.
...
For more details check the documentation [here](https://learn.genesis.global/docs/previous/database/fields-tables-views/fields/tables-advanced/?_highlight=auditing#audit-tables).
